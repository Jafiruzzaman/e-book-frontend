...
const ResponsiveMenu = ({ showMenu }) => {
  return (
    <nav
      className={`${showMenu ? "-left-0" : "-left-[100%]"} z-50  px-4 py-6 transition-all duration-300  top-0 bg-white w-[50%] h-[100vh] absolute`}
    >
      {/* logo */}
        ...
        {links.map((link, index) => {
          return (
            <Link
              key={index}
              href={""}
              className="text-blackPrimary font-medium text-[17px] capitalize hover:text-bluePrimary"
            >
              {link.name}
            </Link>
          );
